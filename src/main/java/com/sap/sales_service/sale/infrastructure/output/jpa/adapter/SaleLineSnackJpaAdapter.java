...
@Component
@AllArgsConstructor
public class SaleLineSnackJpaAdapter implements FindSaleLineSnackPort, SaveSaleLineSnackPort, SnackReportByCinema, SnackSalesReportPort {
    ...
    private final SnackSalesSummaryMapper snackSalesSummaryMapper;
    ...
    @Override
    public List<SnackSalesSummaryDTO> getSnackSalesSummary(LocalDateTime from, LocalDateTime to, UUID cinemaId) {
        return saleLineSnackEntityRepository.findSnackSalesSummary(from, to, cinemaId).stream()
                .map(snackSalesSummaryMapper::toDomain)
                .toList();
    }
}
